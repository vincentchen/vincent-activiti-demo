...
<head>
    ...
    <script src="${ctx }/js/common/plugins/jui/extends/timepicker/jquery-ui-timepicker-addon.js"
        ...
        <tr id="${leave.id }" tid="${task.id }">
            ...
            <td>
                <c:if test="${empty task.assignee }">
                    <a class="claim btn" href="${ctx }/oa/leave/task/claim/${task.id}"><i
                            class="icon-eye-open"></i>签收</a>
                </c:if>
                <c:if test="${not empty task.assignee }">
                    <%-- 此处用tkey记录当前节点的名称 --%>
                    <a class="handle btn" tkey='${task.taskDefinitionKey }' tname='${task.name }' href="#"><i
