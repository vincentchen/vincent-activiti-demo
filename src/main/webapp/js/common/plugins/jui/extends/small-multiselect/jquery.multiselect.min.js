...
(function (d) {
    var o = 0;
    d.widget("ech.multiselect", {options: {header: true, height: 175, minWidth: 225, classes: "", checkAllText: "Check all", uncheckAllText: "Uncheck all", noneSelectedText: "Select options", selectedText: "# selected", selectedList: 0, show: "", hide: "", autoOpen: false, multiple: true, position: {}}, _create: function () {
        var a = this.element.hide(), b = this.options;
        this.speed = d.fx.speeds._default;
        this._isOpen = false;
        a = (this.button = d('<button type="button"><span class="ui-icon ui-icon-triangle-2-n-s"></span></button>')).addClass("ui-multiselect ui-widget ui-state-default ui-corner-all").addClass(b.classes).attr({title: a.attr("title"), "aria-haspopup": true}).insertAfter(a);
        ...
        this.element.find("option").each(function (k) {
            ...
            if (n.length > 0) {
                ...
                d('<input type="' + (b.multiple ? "checkbox" : "radio") + '" ' + (this.selected ? 'checked="checked"' : "") + ' name="multiselect_' + h + '" />').attr({id: k, checked: this.selected, title: m, disabled: i, "aria-disabled": i, "aria-selected": this.selected}).val(n).appendTo(l).after("<span>" + m + "</span>")
            }
        });
        ...
        e.bind({click: a, keypress: function (c) {
            switch (c.which) {
                case 27:
                case 38:
                case 37:
        ...
        this.menu.delegate("li.ui-multiselect-optgroup-label a", "click.multiselect",function (c) {
            ...
            b._trigger("optgrouptoggle", c, {inputs: f.get(), label: g.parent().text(), checked: f[0].checked}) !== false && b._toggleChecked(f.filter(":checked").length !== f.length, f)
                ...
                if (g.is(":disabled") || b._trigger("click", c, {value: f, text: this.title, checked: h}) === false)c.preventDefault(); else {
        ...
        if (e.length)e.find("label").trigger("mouseover"); else {
        ...
        e.not(":disabled").attr({checked: a, "aria-selected": a});
        this.update();
        ...
        }).attr({selected: a, "aria-selected": a})
    }, _toggleDisabled: function (a) {
        this.button.attr({disabled: a, "aria-disabled": a})[a ? "addClass" : "removeClass"]("ui-state-disabled");
        this.menu.find("input").attr({disabled: a, "aria-disabled": a}).parent()[a ? "addClass" : "removeClass"]("ui-state-disabled");
        ...
        if (!(this._trigger("beforeopen") === false || a.hasClass("ui-state-disabled") || this._isOpen)) {
            ...
            } else b.css({top: h.top + a.outerHeight(), left: h.left}).show(f, e);
